...
const FONT = {
  ...
  "_":["00000","00000","00000","00000","00000","00000","11111"],"-":["00000","00000","00000","11111","00000","00000","00000"],"#":["01010","11111","01010","01010","11111","01010","00000"],"/":["00001","00010","00010","00100","01000","01000","10000"],".":["00000","00000","00000","00000","00000","01100","01100"]," ":["00000","00000","00000","00000","00000","00000","00000"]
};
...
function textWidth(text, s) { return clean(text).length * (5 * s + s); }
function drawText(ctx, text, x, y, s, color, maxW = 9999, align = "left") {
  ...
  for (const ch of text) {
    ...
    for (let r=0;r<m.length;r++) for (let c=0;c<m[r].length;c++) if (m[r][c] === "1") ctx.fillRect(cx+c*s+s, y+r*s+s, s, s);
    cx += cw + gap;
  }
  cx = x;
  ctx.fillStyle = color;
  for (const ch of text) {
    const m = FONT[ch] || FONT[" "];
    for (let r=0;r<m.length;r++) for (let c=0;c<m[r].length;c++) if (m[r][c] === "1") ctx.fillRect(cx+c*s, y+r*s, s, s);
    cx += cw + gap;
  }
}
...
async function createLevelUpCard(userData, avatarUrl) {
  ...
  const labelY=752, barX=420, barY=742, barW=760, barH=36;
